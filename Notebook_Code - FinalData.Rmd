...
no_streams_nonan <- select(x, -one_of('Streams'))
...
bivariate_df1 <- select(x, c('Streams', 'viewsCount'))
...
pc_cov <- prcomp(dplyr::select(numeric_cor_x, audio_features))
...
loading_scores <- pc_cor$rotation
loading_scores

library(corrplot)

corrplot(loading_scores, method="color", col=col(200), 
         addCoef.col = "black", # Add coefficient of correlation
         tl.col="black", tl.srt=90, #Text label color and rotation
...
library("psych")
ppc_none<-principal(dplyr::select(numeric_cor_x, audio_features), rotate="none")
ppc_none

par(mfrow=c(1,2))

scree(dplyr::select(numeric_cor_x, audio_features))
...
ppc_varimax<-principal(dplyr::select(numeric_cor_x, audio_features), rotate="varimax")
ppc_varimax
...
par(mfrow=c(1,2))
plot(pc_cor_pred[, 1:2], col = my_colors)
plot(pc_cor$rotation[, 1:2])

plot(pc_cor_pred[, 1:2], type = "n", xlab = "PC1", ylab = "PC2")
text(pc_cor_pred[, 1:2], as.character(x$Genre), col = my_colors)

# Total variance of audiofeatures:

sum(diag(var(scale(dplyr::select(numeric_cor_x, audio_features))))) # gives the total number of variables

ev <- psych::scree(scale(dplyr::select(numeric_cor_x, audio_features))) # important to use standardized data for Kaiser criterion
ev
...
cumsum(ev$pcv)/length(audio_features) # identical
...
test <- psych::principal(scale(dplyr::select(numeric_cor_x, audio_features)), nfactors=2, rotate="none", residuals = TRUE)
...
t1 <- scale(dplyr::select(numeric_cor_x, audio_features))%*%pc_cor$rotation[,1] # N x K
t2 <- scale(dplyr::select(numeric_cor_x, audio_features))%*%pc_cor$rotation[,2]
...
residual <- scale(dplyr::select(numeric_cor_x, audio_features)) - fit
